...
def match(resultsfile, groundtruthfile, show_images):
    ...
    for i in range(len(results)):
        ...
        if show_images:  # 若需可视化
            ...
            fname = 'Test\INA-T/' + results[i][0]   # 若需可视化, 修改这里为存放图片的路径
            print(fname)
        ...
        for j in range(2, len(results[i])):  # 对于一张图片中的每一个检测框
            ...
            if show_images:  # 若需可视化
                ...
                cv2.putText(image, text='detectedbox------', org=(3, 15), fontFace=cv2.FONT_HERSHEY_SIMPLEX,
            ...
            for k in range(2, len(groundtruth[i])):  # 去匹配这张图片中的每一个标注框
                ...
                if show_images:  # 若需可视化
                    ...
                    cv2.putText(image, text='groundtruthbox-----', org=(3, 30), fontFace=cv2.FONT_HERSHEY_SIMPLEX,
...
def thres(maxiou_confidence, threshold = 0.5):
    ...
    print('############################################################')
    print('mean_ious：', np.mean(maxious))
...
resultsfile = 'D:\work//new\Test//result//detectedbox.txt'
groundtruthfile = 'D:\work//new\Test//result\groundtruthbox.txt'
draw_curves(resultsfile, groundtruthfile, show_images=False, threshold=0.5)
